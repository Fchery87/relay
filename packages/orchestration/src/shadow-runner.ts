...
function isComparableEvent(type: string): boolean {
  return [
    ...
    "checkpoint.captured", "checkpoint.restored", "checkpoint.compared", "workspace.diff.updated", "review.comment.created", "review.comment.resolved", "projection.published",
  ].includes(type);
}

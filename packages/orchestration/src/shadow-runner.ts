...
function isComparableEvent(type: string): boolean {
  return [
    ...
    "checkpoint.captured", "checkpoint.restored", "checkpoint.compared", "workspace.diff.updated", "git.action.updated", "run.configuration.updated", "plan.updated", "review.comment.created", "review.comment.resolved", "projection.published",
  ].includes(type);
}

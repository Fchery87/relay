...
Manually opened the published PRD and compared it with the `to-spec` template, the local issue-tracker conventions, the `ready-for-agent` triage vocabulary, the domain glossary, and ADRs 0001–0003.

Observed outcome:

- The canonical PRD is published at `.scratch/self-hosted-convex-recovery/PRD.md`; a numbered implementation issue is also published in the existing harness-kernel effort.
- The PRD is labeled `Status: ready-for-agent`.

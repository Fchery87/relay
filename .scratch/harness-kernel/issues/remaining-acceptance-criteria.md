...
- [x] Opt-in real-Codex e2e smoke test (`RELAY_E2E_CODEX=1`, skipped in ordinary CI; local ChatGPT-login runs may set `RELAY_CODEX_HOME=$HOME/.codex`, while protected CI continues using its injected `OPENAI_API_KEY`) — local pass recorded with seeded isolated `CODEX_HOME`, `bun run scripts/smoke-codex-harness.ts`

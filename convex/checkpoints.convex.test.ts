...
test("records checkpoints and machine-scopes queued restores", async () => {
  ...
  const { deviceToken, owner, projectId, userId } = await createAuthenticatedProject(t);
  const otherDeviceToken = `${"e".repeat(31)}2`;
  const otherDeviceTokenHash = await digestSecret(otherDeviceToken);
  await t.run((ctx) => ctx.db.insert("machines", {
    daemonVersion: "test",
    deviceTokenHash: otherDeviceTokenHash,
    lastHeartbeatAt: Date.now(),
    name: "other-machine",
    ownerId: userId,
    platform: "linux",
  }));
  const { checkpointThreadId, messageId, otherThreadId } = await t.run(async (ctx) => {
    const checkpointThreadId = await ctx.db.insert("threads", { projectId, status: "idle", title: "checkpoint" });
    const otherThreadId = await ctx.db.insert("threads", { projectId, status: "idle", title: "other" });
    const messageId = await ctx.db.insert("messages", { content: "done", role: "assistant", status: "complete", threadId: checkpointThreadId });
    return { checkpointThreadId, messageId, otherThreadId };
  });
  ...
  expect(await owner.query(api.checkpoints.listForThread, { threadId: checkpointThreadId })).toMatchObject([
  ...
  const comparisonId = await owner.mutation(api.checkpoints.enqueueComparison, { fromCheckpointId: checkpointId, threadId: checkpointThreadId, toCheckpointId: secondCheckpointId });
  ...
  expect(await owner.query(api.checkpoints.latestComparison, { threadId: checkpointThreadId })).toMatchObject({ content: "patch", status: "complete" });
  await expect(owner.mutation(api.checkpoints.enqueueRestore, { checkpointId, threadId: otherThreadId })).rejects.toThrow("Checkpoint does not belong to thread");

  const actionId = await owner.mutation(api.checkpoints.enqueueRestore, { checkpointId, threadId: checkpointThreadId });
  expect((await t.run((ctx) => ctx.db.get("threads", checkpointThreadId)))?.status).toBe("restoring");
  await owner.mutation(api.conversations.sendUserMessage, { content: "wait until restore finishes", threadId: checkpointThreadId });
  expect((await t.run((ctx) => ctx.db.get("threads", checkpointThreadId)))?.status).toBe("restoring");
  expect(await t.mutation(api.conversations.claimQueuedMessage, { deviceToken })).toBeNull();
  await expect(owner.mutation(api.commands.enqueue, { command: "touch race.txt", threadId: checkpointThreadId })).rejects.toThrow("restore");
  await expect(owner.mutation(api.git_actions.enqueue, { action: "stage", threadId: checkpointThreadId })).rejects.toThrow("restore");
  expect(await t.mutation(api.checkpoints.claimRestore, { deviceToken: otherDeviceToken })).toBeNull();
  ...
  expect((await t.run((ctx) => ctx.db.get("threads", checkpointThreadId)))?.status).toBe("restoring");
  await expect(t.mutation(api.checkpoints.completeRestore, { actionId, claimToken: secondRestoreClaim!.claimToken, deviceToken: otherDeviceToken, status: "complete" })).rejects.toThrow("does not own");
  ...
  expect(await owner.query(api.events.list, { threadId: checkpointThreadId })).toMatchObject([
    { checkpointId, kind: "checkpoint.reverted" },
  ]);

  await owner.mutation(api.conversations.removeThread, { threadId: checkpointThreadId });

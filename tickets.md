...
- [x] Restore validates schema deployment, restored password sign-in, pairing, ownership, file storage, and projection-metrics reads through `scripts/restore-acceptance.ts`; a disposable full-backup rehearsal passed on 2026-07-23 and is documented in `docs/operations/backup-recovery.md`. Live account ownership and device-heartbeat recovery were separately rehearsed against restored data. Kernel local-store reopen and non-empty projection reconciliation remain release-window evidence because this development backup had no kernel history to exercise.
...
- [ ] Ordinary CI uses deterministic effects; protected jobs cover the real provider and supported topology — the ordinary daemon suite passes with 341 tests and 16 intentional protected-profile skips; the protected `real-codex-harness` job exists (`.github/workflows/ci.yml`, `workflow_dispatch`-only) and the harness isolates `CODEX_HOME` by default while supporting an explicit `RELAY_CODEX_HOME` for deliberate local authenticated runs, but this item remains open until a credentialed run records the real provider lifecycle. The isolated `cross-tier-recovery` job was freshly rerun at `e20c825` with 13/13 tests and 51 assertions; explicit opt-in now fails if its backend or loopback prerequisites are missing instead of going green empty.

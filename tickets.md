...
- [ ] Ordinary CI uses deterministic effects; protected jobs cover the real provider and supported topology — the ordinary daemon suite passes with 338 tests and 16 intentional protected-profile skips; the protected `real-codex-harness` job exists (`.github/workflows/ci.yml`, `workflow_dispatch`-only) and the harness isolates `CODEX_HOME`, but this item remains open until a credentialed run records the real provider lifecycle. The separate `cross-tier-recovery` job passed 13/13 with 51 assertions on the final tree (`8fb9c1a`); explicit opt-in now fails if its backend or loopback prerequisites are missing instead of going green empty.

...
- [ ] Ordinary CI uses deterministic effects; protected jobs cover the real provider and supported topology — ordinary `bun run test` now passes with 325 tests and only capability-dependent skips (the live Convex and MCP fixtures skip when their binary/loopback/process capability is unavailable); the protected `real-codex-harness` job exists (`.github/workflows/ci.yml`, `workflow_dispatch`-only) and the harness now isolates `CODEX_HOME`, but this item remains open until a credentialed run records the real provider lifecycle; the separate `cross-tier-recovery` job still covers only the real self-hosted backend

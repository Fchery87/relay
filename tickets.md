...
- [ ] Scenario covers create, resume, send, streaming, approval, steer/interrupt, checkpoint, projection, disconnect, and reconnect — create/resume/send/streaming/projection/disconnect/reconnect are proven live (`apps/daemon/src/cross-tier-recovery.e2e.test.ts`). Command *routing* for approval/steer/interrupt is now proven too — `turn.steer`, `turn.interrupt`, and `approval.resolve` were previously missing from `KernelDaemon`'s dispatch switch entirely (silently rejected as "unknown command kind"); fixed, and live tests confirm correct precondition rejection (steering after a turn ends, resolving a nonexistent approval). Real approval *creation* and mid-turn steering delivery remain structurally absent — see `docs/operations/kernel-mode-capability-gaps.md`. Checkpoint is still not covered (needs workspace/git-worktree state, see the item above)
- [ ] Duplicate/conflicting commands, lease expiry, stale completion, lost response, daemon/backend restart, and projection duplicate/reorder/partial publication are injected — duplicate/conflicting commands, lease expiry + redelivery, stale-worker fencing, daemon restart, and backend process restart are now all proven against the *real* seam (`apps/daemon/src/cross-tier-recovery.e2e.test.ts`, 9 passing tests). Finding along the way: `commands/inbox:submitToInbox`'s conflict check didn't compare `payloadJson`, so two different payloads under the same `commandId` would silently return the original receipt instead of being rejected as conflicting — fixed. Still not covered against the real seam: lost response (client-side timeout while the server-side mutation may have committed) and projection duplicate/reorder/partial publication (covered only against fakes in `kernel-daemon.projection-outbox.test.ts`)

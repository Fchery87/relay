...
- [x] Scenario covers create, resume, send, streaming, approval, steer/interrupt, checkpoint, projection, disconnect, and reconnect — create/resume/send/streaming/projection/disconnect/reconnect, explicit checkpoint restore, and command routing/precondition rejection for approval/steer/interrupt are proven live in `apps/daemon/src/cross-tier-recovery.e2e.test.ts`. Real approval creation, governance/tool execution, mid-turn steering, and actual interrupt cancellation remain structurally absent by the scoped kernel capability finding in `docs/operations/kernel-mode-capability-gaps.md`

...
- [ ] Ordinary CI uses deterministic effects; protected jobs cover the real provider and supported topology — ordinary daemon CI now passes with 345 tests and one credential-dependent skip; the protected `real-codex-harness` job exists (`.github/workflows/ci.yml`, `workflow_dispatch`-only) and the harness now isolates `CODEX_HOME`, but this item remains open until a credentialed run records the real provider lifecycle; the separate `cross-tier-recovery` job covers the real self-hosted backend and passed 13/13 on commit `f4786ea`

...
**Cutover gates:** the code-level gate evaluator requires `kernelReady`, `releaseWindowSatisfied`, `backupRehearsalVerified`, `acceptanceGatesPassed`, and `zeroLegacyActivations` before an implicit kernel default is allowed. The checked-in `DEFAULT_GATES` remains fail-closed until those gates are supplied by verified release evidence; the effective default remains `legacy`.
